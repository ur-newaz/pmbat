...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (mysqli_num_rows($check_result) > 0) {
        $sql = "update toll set amount = '$toll_amount' where type = '$vehicle_type'";
        
        if (mysqli_query($conn, $sql)) {
            $message = "Toll entry for vehicle type '$vehicle_type' updated successfully!";
        } else {
            $message = "Error updating toll entry: " . mysqli_error($conn);
        }
    } else {
...
<body>
    ...
    <h1>Update Toll Entry</h1>

    <form method="POST">
        <input type="hidden" name="vehicle_type" value="<?php echo htmlspecialchars($vehicle_type); ?>">
        <label for="toll_amount">New Toll Amount:</label>
        <input type="number" id="toll_amount" name="toll_amount" value="<?php echo htmlspecialchars($toll_amount); ?>" required>
        
        <p>Vehicle Type: <strong><?php echo htmlspecialchars($vehicle_type); ?></strong></p>
        
        <button type="submit">Confirm Update Toll</button>
    </form>
